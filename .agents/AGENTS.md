...
### 🏆 Главный критерий готовности (Definition of Done)
> ⚠️ **Результат выполнения задачи — это НЕ просто написанный или скомпилированный код, а то, что новая версия приложения реально РАБОТАЕТ в Kubernetes.**
- Задача считается завершенной **ТОЛЬКО** при выполнении следующих условий:
  1. Под приложения поднялся в Kubernetes и находится в состоянии `Running`.
  2. Запросы к эндпоинтам `/actuator/health/liveness` и `/actuator/health/readiness` отдают HTTP 200 `UP`.
  3. **СТРОГИЙ ЗАПРЕТ И ОБЯЗАТЕЛЬНЫЙ ТАЙМЕР (SCHEDULE)**:
     - Запрещено утверждать, что ошибок нет, или закрывать задачу в Plane, пока приложение не отработает минимум **5 ПОЛНЫХ МИНУТ** в состоянии `Running` с непрерывно чистыми логами (0 неперехваченных ошибок/исключений за 5 минут).
     - Если под поднялся недавно и до 5 минут нужно подождать (например, 2-4 минуты), AI-агент **ОБЯЗАН поставить фоновый таймер через инструмент `schedule`** на оставшееся время ожидания и категорически не имеет права уходить в спячку или перекладывать ожидание на пользователя.

### 🚫 СТРОГИЙ ЗАПРЕТ НА ИСПОЛЬЗОВАНИЕ IP-АДРЕСОВ (ТОЛЬКО K8S DNS SERVICE NAMES!)
> ⚠️ **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** использовать IP-адреса в коде, K8s-манифестах, initContainers, конфигах или скриптах для подключения к сервисам или базам данных.
- **ВСЕГДА ИСПОЛЬЗОВАТЬ ТОЛЬКО K8S DNS ИМЕНА СЕРВИСОВ** (например: `igaming-source-winline-db`, `igaming-source-winline-db.igaming-source.svc.cluster.local`).
- В initContainers / bash-скриптах ожидать готовность сервисов **ТОЛЬКО по их DNS-имени**, используя повторы подключения (`until psql -h "$SERVICE_DNS_NAME" ...; do sleep 2; done`).

### 🛠️ Протокол проверки исходного приложения и авторемонта (Repair Protocol)
Перед началом выполнения любой фиче-задачи AI-агент **обязан** проверить работоспособность базового модуля:
1. **Если базовое приложение уже сломано** (`CrashLoopBackOff`, ошибки подключения к БД, битая конфигурация):
   - **Запрещено** писать код фичи поверх сломанного фундамента!
   - Агент приостанавливает текущую задачу в Plane.
   - Агент автоматически создает отдельную задачу на ремонт в Plane: `[REPAIR] Восстановление работоспособности модуля <module>` (ключ `IGAMING-FIX-<MODULE>`).
   - Агент создает ветку ремонта `fix/<module>` и переводит ремонтную задачу в статус **`AI разработка`**.
   - Отдельный агент чинит базовое приложение до 100% зеленого статуса в K8s.
   - После завершения ремонта фиче-задача возобновляется на заведомо рабочем модуле.

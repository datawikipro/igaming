    ...
    public Map<String, ServerInfo> listServers(CloudCredential cred) {
        ...
        try {
            ...
            for (JsonNode inst : node.path("instances")) {
                ...
                String status = inst.path("status").asText().toUpperCase(); 
                String powerStatus = inst.path("power_status").asText().toUpperCase(); 
            ...
            log.error("Failed to fetch Vultr instances info for cred: {}", cred.getId(), e);
        }
        return infoMap;
    }

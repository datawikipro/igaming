...
curl -sfL https://get.k3s.io | \
  ...
  INSTALL_K3S_EXEC="agent --node-label=node-type=spot --node-label=role=crawler" \
  sh -

echo "K3s agent installed and joined cluster as SPOT" >> /var/log/startup-script.log

...
if [ "$ONLY" == "build-base" ]; then
    ...
    echo -e "\033[1;30m  > Building build-base locally...\033[0m"
    if ! docker build -f Dockerfile.build-base -t "$IMAGE_TAG" .; then
        echo -e "\033[0;31mFATAL: Build-base image build failed!\033[0m"
        exit 1
    fi
    if ! docker push "$IMAGE_TAG"; then
        echo -e "\033[0;31mFATAL: Build-base image push failed!\033[0m"
        exit 1
    fi
...
for module in "${SUCCESS[@]}"; do
    if [[ "$module" == igaming-source-* ]]; then
        ...
        if [ "$module" == "igaming-source-fon-bet-ru" ]; then
            ...
            DEPLOYMENT_PREFIXES=("igaming-source-1xbet" "igaming-source-melbet" "igaming-source-megapari" "igaming-source-linebet" "igaming-source-betandyou" "igaming-source-fansport" "igaming-source-888starz")
        else

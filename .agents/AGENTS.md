...
- Задача считается завершенной **ТОЛЬКО** при выполнении следующих условий:
  ...
  3. **СТРОГИЙ ЗАПРЕТ**: Запрещено утверждать, что ошибок нет, или закрывать задачу в Plane, пока приложение не отработает минимум **5 ПОЛНЫХ МИНУТ** в состоянии `Running` с непрерывно чистыми логами (0 неперехваченных ошибок/исключений за 5 минут).

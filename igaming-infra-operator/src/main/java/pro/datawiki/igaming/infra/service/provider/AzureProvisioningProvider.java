...
public class AzureProvisioningProvider implements CloudProvisioningProvider {
    ...
    public String provisionNode(CloudAccount account, String model, String role, String machineType, String region, String zone) {
        ...
        String subId = getApiKey(account, "subscriptionId");
        String rg = getApiKey(account, "resourceGroup");
        ...
        // Format Azure Virtual Machine creation request body
                ...
                "linuxConfiguration", Map.of(
                        ...
                        "ssh", Map.of("publicKeys", java.util.List.of(
                                Map.of(
                                        ...
                                        "keyData", getApiKey(account, "sshPublicKey")
                                )
                        ))
                )
        ));
        ...
        try {
            // Note: In real production code, we must create a Network Interface (NIC) before creating the VM,
            // or let Azure do it. Here we issue the PUT for the VM.
            String response = armClient.put()
    ...
    public Map<String, CloudNodeInfo> getInstancesInfo(CloudAccount account) {
        ...
        try {
            ...
            for (JsonNode vm : node.path("value")) {
                ...
                // To keep it lightweight, we check VM status under provisioningState
            ...
            log.error("Failed to fetch Azure VM instances info for account: {}", account.getName(), e);
        }
        return infoMap;
    }
    ...
    private String getAccessToken(CloudAccount account) {
        String tenantId = getApiKey(account, "tenantId");
        String clientId = getApiKey(account, "clientId");
        String clientSecret = getApiKey(account, "clientSecret");

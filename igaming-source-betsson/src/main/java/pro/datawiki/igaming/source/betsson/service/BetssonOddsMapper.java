...
import pro.datawiki.igaming.source.betsson.dto.kambi.KambiBetOffer;
...
import pro.datawiki.igaming.source.betsson.dto.kambi.KambiOutcome;
...
public class BetssonOddsMapper {
    ...
    public OddsUpdateRequest mapToOddsUpdateRequest(KambiEventDetailsResponse response, String fallbackSport, String fallbackLeague) {
        ...
        request.setEventUrl("https://www.betsson.com/betting/sports/event/" + event.getId());
    ...
    private BetType resolveBetType(KambiBetOffer betOffer, KambiOutcome outcome, SportType sportType, 
        ...
        return betTypeResolver.resolve("betsson", sportType, mUpper, runnerName.toUpperCase(), line);
    }

    private void logUnmapped(KambiEvent event, String sportName, String marketName, String runnerName) {
        log.debug("UNMAPPED BETSSON MARKET: Event={}, Sport={}, Market={}, Runner={}",
                event.getId(), sportName, marketName, runnerName);
        unmappedBetService.saveAndNotify("betsson", sportName, runnerName, marketName, String.valueOf(event.getId()));
    }
}

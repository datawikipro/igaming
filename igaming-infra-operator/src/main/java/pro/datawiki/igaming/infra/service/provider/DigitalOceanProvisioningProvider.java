...
public class DigitalOceanProvisioningProvider implements CloudProvisioningProvider {
    ...
    public String provisionNode(CloudAccount account, String model, String role, String machineType, String region, String zone) {
        String token = getApiKey(account, "token");
        String prefix = "SPOT".equalsIgnoreCase(model) ? "spot-" : "std-";
        String dropletName = prefix + account.getProvider().toLowerCase() + "-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    public Map<String, CloudNodeInfo> getInstancesInfo(CloudAccount account) {
        ...
        try {
            ...
            for (JsonNode droplet : node.path("droplets")) {
                ...
                String status = droplet.path("status").asText().toUpperCase(); // "active", "new", "off", "archive"
                // Map status to K8s reconcile status
            ...
            log.error("Failed to fetch DigitalOcean instances info for account: {}", account.getName(), e);
        }
        return infoMap;
    }
